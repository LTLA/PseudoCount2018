...
In practice, we need to compare many cells to one another instead of just two specific cells.
The exact choices of $s_1$ and $s_2$ are not obvious when considering a single pseudo-count for multiple cells.
...
This ensures that the difference in log-values for a non-DE gene will be capped below the upper bound for pairwise comparisons between any pair of cells.

\section{Examining effects on power}
