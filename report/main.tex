...
This avoids spurious non-zero differences in expression due to the log-transformation while accounting for complex mean-variance relationships in the count data.
...
\subsection{Pseudo-count calculations for real scRNA-seq data}
Count data from various scRNA-seq experiments were processed according to a published workflow \cite{lun2016stepbystep}.
...
Values of $\hat c$ were then computed for each data set as previously described.
For comparison, we also computed the mean of the normalized expression values for each gene.
